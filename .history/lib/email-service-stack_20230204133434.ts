...
// import { Construct as B} from '@aws-cdk/core';

// import { Construct as X} from 'constructs';
import { Construct as CoreConstruct } from '@aws-cdk/core';
import { Construct as ConstructsConstruct } from 'constructs';
...
export class EmailServiceStack extends Stack {
  constructor(scope: ConstructsConstruct, id: string, props?: StackProps) {
    ...
    const lambdaFn = new lambda.Function(this, 'ProcessBirthDayLambda', {
      code: lambda.Code.fromAsset('lambda'),
    ...
    // const construct: B = this;
    // const coreConstruct: CoreConstruct = this;
    const constructsConstruct: ConstructsConstruct = this;

  // Create the CloudWatch Event Rule with a cron expression
  const eventRule = new Rule(coreConstruct, "fiveMinuteRule", {
    schedule: Schedule.cron({ minute: "0/5" }),
  });

  // Add the Lambda function as a target for the Event Rule

  const rule = new events.Rule(constructsConstruct, 'Schedule Rule', {
